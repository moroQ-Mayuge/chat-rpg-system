ALTER TABLE items ADD COLUMN outfit_master_id INTEGER REFERENCES outfit_masters(id) ON DELETE SET NULL;

-- command_type の CHECK 制約に 'item_wear' を追加するため、0009と同じ
...
CREATE TABLE action_commands_new (
  ...
  command_type TEXT NOT NULL CHECK (command_type IN ('keyword', 'item_pickup', 'item_check', 'item_use', 'item_wear', 'free_text')),
