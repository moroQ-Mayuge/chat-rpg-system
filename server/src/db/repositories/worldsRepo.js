...
function parseWorld(row) {
  ...
  return {
    ...
    undress_image_generation_enabled: Boolean(row.undress_image_generation_enabled),
    outfit_change_image_generation_enabled: Boolean(row.outfit_change_image_generation_enabled),
    ...
    address_auto_update_enabled: Boolean(row.address_auto_update_enabled),
...
export function createWorld({
  ...
  // 'off'=従来通り(ペルソナ無し、モブは同行不可) / 'preset'=プリセット抽選 / 'llm'=LLM都度生成。
  // 旧mob_random_flavor_enabled+mob_flavor_generation_modeの2列は0128で統合済み、以後未使用。
  mob_flavor_mode = 'off',
    ...
    .prepare(
      `INSERT INTO worlds
        (name, worldview, is_unassigned_bucket, time_slot_labels, weather_options, season_labels, days_per_season, day_of_week_labels, holiday_weekday_indices, image_style_preset_id, image_tags,
         ...
         mob_flavor_mode, call_method_enabled, call_method_lore, player_state_tracking_enabled, address_auto_update_enabled)
       VALUES (?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    )
    .run(
      name,
      ...
      conversation_summary_interval_turns ?? null,
      session_boundary_mode ?? 'time_slot',
      session_boundary_defer_to_move ? 1 : 0,
      session_max_turns ?? null,
      undress_image_generation_enabled ? 1 : 0,
      ...
      address_auto_update_enabled ? 1 : 0,
    );
...
export function updateWorld(
  id,
  {
    ...
    session_boundary_defer_to_move,
    session_max_turns,
    undress_image_generation_enabled,
    outfit_change_image_generation_enabled,
  ...
  db.prepare(
    `UPDATE worlds
     SET name = ?, worldview = ?, time_slot_labels = ?, weather_options = ?, season_labels = ?, days_per_season = ?, day_of_week_labels = ?, holiday_weekday_indices = ?, image_style_preset_id = ?, image_tags = ?,
         ...
         debug_accompany_toggle_enabled = ?, mob_flavor_mode = ?, call_method_enabled = ?, call_method_lore = ?, player_state_tracking_enabled = ?, address_auto_update_enabled = ?
