...
import { generateChatCompletion, generateImage } from '../services/koboldClient.js';
import { buildSceneTagPrompt, buildReferenceAnchorCanvas, cropMainRegion, suggestSceneTags, MAIN_WIDTH, MAIN_HEIGHT } from '../services/imagePromptBuilder.js';
...
async function generateSceneImage(sessionId, sceneChangeDescription) {
  ...
  const tagPrompt = buildSceneTagPrompt(updatedSession, updatedSession.participants);
  const prompt = [stylePrompt, tagPrompt].filter(Boolean).join(', ');
