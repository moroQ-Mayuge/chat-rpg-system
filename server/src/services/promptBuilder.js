...
function buildSystemPrompt(session, participants, options = {}) {
  ...
  return [
    ...
    options.isCraftAttempt
      ? `[CRAFT_RESULT: 完成品名|カテゴリ名|消費型または永続型|個数]: 直前のプレイヤー発言にある道具・材料・使い方を踏まえ、実際に何が出来上がったかを描写した上で、必ずこの行を追加してください。4つの項目は必ず全て書いてください（例：[CRAFT_RESULT: 野菜スープ|${itemCategoryNames[0] ?? '食料'}|消費型|2]）。指定された使い方がその道具では現実的に不可能な場合（例：オーブンで「煮る」）は、CRAFT_RESULTを付けず「それは無理そうだ」という趣旨の失敗として描写してください。それ以外（使い方として筋が通っている場合）は、組み合わせが極端に不合理でない限り多少不出来でも何かを完成させてください。3つ目は、料理・食べ物・薬など使うと無くなる物なら「消費型」、道具・装備など使っても残る物なら「永続型」と書いてください（料理は必ず「消費型」です）。4つ目は出来上がった個数で、材料が大量にある場合やクッキー・団子など数えられる物が複数できた場合はその数を書いてください（1個なら1）。カテゴリ名は次のいずれかから選んでください：${itemCategoryNames.join(', ')}`
      : null,
