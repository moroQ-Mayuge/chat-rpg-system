...
function buildSystemPrompt(session, participants, options = {}) {
  ...
  return [
    ...
    options.isCraftAttempt
      ? `[CRAFT_RESULT: 完成品名|カテゴリ名|消費型または永続型]: 直前のプレイヤー発言にある道具・材料・使い方を踏まえ、実際に何が出来上がったかを描写した上で、必ずこの行を追加してください。指定された使い方がその道具では現実的に不可能な場合（例：オーブンで「煮る」）は、CRAFT_RESULTを付けず「それは無理そうだ」という趣旨の失敗として描写してください。それ以外（使い方として筋が通っている場合）は、組み合わせが極端に不合理でない限り多少不出来でも何かを完成させてください。3つ目の項目は、料理・薬など使うと無くなる物なら「消費型」、道具・装備など使っても残る物なら「永続型」と書いてください。カテゴリ名は次のいずれかから選んでください：${itemCategoryNames.join(', ')}`
      : null,
