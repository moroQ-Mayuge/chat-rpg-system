...
// { character_id: number|"all_present"|"mentioned"|"condition_matched", axis_id, operation: "add"|"subtract"|"set", value, mentioned_limit? }
export async function executeChangeRelationship(params, execCtx) {
  ...
  // reads each participant's own instance id directly (via the participant
  // row, not this lookup); every other sentinel consults
  // instanceHintByCharacterId (explicit @mention, falling back to whichever
  // instance most recently spoke this turn -- see roomSessions.js's
  // generateReply). instance_id is undefined when no hint exists, which
  // relationshipStatesRepo.js treats as "apply without instance scoping"
  // (today's shared-across-instances behavior).
  const targets =
