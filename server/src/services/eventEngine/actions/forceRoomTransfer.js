import { endSessionForMove, createRoomSession } from '../../../db/repositories/roomSessionsRepo.js';
...
export function executeForceRoomTransfer(params, execCtx) {
  ...
  const carryOverParticipants = execCtx.session.participants
    .filter((p) => carryIds.has(p.character_id))
    .map((p) => ({ character_id: p.character_id, current_outfit_id: p.current_outfit_id }));

  endSessionForMove(execCtx.sessionId);
  const newSession = createRoomSession(execCtx.playthroughId, target_room_template_id, {
    carryOverParticipants,
    fromRoomSessionId: execCtx.sessionId,
  });
  broadcast(execCtx.sessionId, { type: 'forced_room_transfer', new_session_id: newSession.id });
